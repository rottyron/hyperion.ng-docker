...
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
    apt-get install -y wget gpg apt-transport-https lsb-release qtbase-5 sudo && \
    ...
    echo "deb [signed-by=/usr/share/keyrings/hyperion.nightly.pub.gpg] https://nightly.apt.hyperion-project.org/ trixie main" > /etc/apt/sources.list.d/hyperion.nightly.list.disabled && \
    apt-get update && \
